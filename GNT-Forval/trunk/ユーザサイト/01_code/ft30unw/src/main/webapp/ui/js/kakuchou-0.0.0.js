 ...
 * 拡張項目：解約ボタン
 */
function kakuchouCancel(targetId){
	// 受注番号と受注明細番号を取得
	var target = $('#' + targetId);	
//	var juchNo = $('#ft-kakuchou-juchno').html();
	var juchNo = $(target).parents('table').parents('.order_div').find("#ft-kakuchou-juchno").html();	
	var juchMeisaiNo = $(target).parents('table').find('.ft-kakuchou-juchmeisaino').html();
	
	// 存在しない場合、処理しない
	if (juchNo == undefined || juchMeisaiNo == undefined) {
		return;
	}
	...
	if ($(target).val() == '解約申請' || $(target).val() == '解約') {
		showConfirm('解約申請登録を行います。', function(){
			// Submit
			submitTargetForm(target, 'contractInfoQuery/cancalOption');
		});
	} else {
		showConfirm('解約解除登録を行います。', function(){
			// Submit
			submitTargetForm(target, 'contractInfoQuery/cancelOptionCancel');
		});
	}
}
 ...
 * サービス拡張項目取得処理
 */
function settingSrvKakuchou(target){

	// 引数の値を取得
//	var juchNo = $('#ft-kakuchou-juchno').html();
	var juchNo = $(target).parents('.order_div').find("#ft-kakuchou-juchno").html();
	var hyoujiKbn = $('#ft-kakuchou-hyoujikbn').html();
	var juchMeisaiNo = $(target).find('.ft-kakuchou-juchmeisaino:first').html();
	var srvCd = $(target).find('.ft-kakuchou-srvcd:first').html();
	//TTM_DEV 20170615 ADD BEGIN
	var screen = $("#ft-kakuchou-screen").val();
	...
	$.ajax({
		...
		data: {
			'juchNo':juchNo,
			'juchMeisaiNo':juchMeisaiNo,
			'srvCd':srvCd,
			'shohinCd':'',
			'hyoujiKbn':hyoujiKbn,
			'mode':$('#ft-readOnlyFlg').html(),
			'screen':screen	
		},
 ...
 * 商品拡張項目設定処理
 */
function settingShohinKakuchou(target)　{
...
//	var juchNo = $('#ft-kakuchou-juchno').html();
	...
	var hyoujiKbn = $('#ft-kakuchou-hyoujikbn').html();
	var juchMeisaiNo = $(target).find('.ft-kakuchou-juchmeisaino:first').html();
	var shohinCd = $(target).find('.ft-kakuchou-shohincd:first').html();
	//TTM_DEV 20170615 ADD BEGIN
	var screen = $("#ft-kakuchou-screen").val();
	//TTM_DEV 20170615 ADD END
	// --- 拡張項目の設定---
...
$(function() {
	...
	$('.ft-kakuchou-check').on('change', function(){
		...
		$(tdCheck).find('.ft-kakuchou-check').each(function(){
			// TTM_DEV-543 20170831 BEGIN UPDATE
//			if (value != '') {
//				value = value + ',';
//			}
...
//				value = value + '0';
//			}
			// TTM_DEV-543 20170831 END UPDATE
		});
	...
	// --- 拡張項目 サービス表示
	$('.ft-kakuchou-service').each(function() {
		// サービス拡張項目設定
		settingSrvKakuchou(this);
	});
